...
export default {
  ...
  methods: {
    ...
    getRequestCode() {
      // alert('+++++++++getRequestCode++++++++++++' + Config.corpId)
      // alert(Config.caseNumber)
      dd.ready(() => {
        dd.runtime.permission.requestAuthCode({
          ...
          onSuccess: info => {
            getUserInfo({ requestAuthCode: info.code }).then(res => { // 通过该免登授权码可以获取用户身份
              // alert('进来的用户为' + res.data.department[0])
              this.loginForm.username = res.data.userid + ''
              this.$store.dispatch('setDingUser', res.data) // 调用store中的set方法把钉钉用户信息保存到vuex中
              this.handleLogin()
            })

            this.loginForm.password = info.code
            this.password = info.code
            this.getRequestCode = info.code
          },
          onFail: err => {
            alert('失败l')
    ...
    getCookie() {
      ...
      this.loginForm = {
        ...
        username: Config.corpId,
        password: this.requestAuthCode,
